...
<?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
		...
		<div class="no-results"><?php _e("Be the first to comment", 'blog'); ?></div>
...
<div id="respond" class="comment-respond">
	<h3 id="reply-title" class="comment-reply-title"><?php _e("Leave a comment", 'blog'); ?>
		<small>
			<a rel="nofollow" id="cancel-comment-reply-link" href="javascript:;" style="display:none;"><?php _e("Cancel reply", 'blog'); ?></a>
		</small>
	</h3>
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" class="comment-form">
		<?php if($user_ID) : ?>  
			<p class="logged-in-as">
				<a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php" aria-label="Logged in as <?php echo $user_identity; ?>. Edit your profile."><?php _e("Logged in as", 'blog'); ?> <?php echo $user_identity; ?></a>.
				<a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account"><?php _e("Log out &raquo;", 'blog'); ?></a>
			</p>
		<?php else : ?>
		...
		<div class="modern-field comment-form-comment">
			...
			<label for="comment">
				<span><?php _e("Comment:", 'blog'); ?></span>
			</label>
		</div>
		
		<p class="form-submit">
			<input name="submit" type="submit" id="submit" class="submit blue-button" value="<?php _e("Post Comment", 'blog'); ?>">
