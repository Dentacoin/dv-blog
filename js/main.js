...
jQuery(document).ready(function($){
    ...
    $('.subscribe-scroll').click( function() {
    	$('html, body').animate({
            scrollTop: $('.subscribe-box').offset().top - 90
        }, 500);
    });

    function share_buttons() {
		$('.share.fb, .share.twt, .share.in, .share.messenger, .share.email').click( function() {
			...
			if ($(this).hasClass('fb')) {
				...
				var url = 'fb-messenger://share?link=' + encodeURIComponent(post_url) + '&app_id=1906201509652855';
		...
		$.ajax({
			...
			success : function(text) {
				if (text) {				
					...
					$('.user-logo img').attr('src', text.thumbnail_url );
