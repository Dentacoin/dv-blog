    ...
    if(isset($_POST['page'])){
        ...
        if ( $all_blog_posts->have_posts() ) {
            while ( $all_blog_posts->have_posts() ) {
                ...
                $c_c_id = '';
                $p_cat = '';
                foreach (wp_get_post_categories( get_the_ID() ) as $k => $cat_id) {
                    if($k == 0) {
                ...
                $image_position = !empty(get_field('featured_image_position')) || get_field('featured_image_position') === '0' ? get_field('featured_image_position').'%' : '50%';

                // Set the desired output into a variable
                $msg .= '
                <div class="news">
                    <a href="' . get_permalink() . '" class="news-image cover" style="background-image: url('.$src[0].'); background-position: 50% '.$image_position.'">            
                        <div class="cat">'.$p_cat.'</div>
                    </a>
                    <a href="'.get_permalink().'"><h4>'.get_the_title().'</h4></a>
                    <a class="new-category" href="'.get_category_link($c_c_id).'">'.$p_cat.'</a>
                    <p>
                        '.get_the_excerpt().'
                    </p>
                    <a href="'.get_permalink().'" class="read-more">Read more<img src="'.get_stylesheet_directory_uri().'/img/read-arrow.png"></a>
                </div>';
            }
        }
